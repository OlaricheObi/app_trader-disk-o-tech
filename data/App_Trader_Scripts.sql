...
-- 				ON aa.name = pa.name
-- 				ORDER BY expected_revenue DESC),
-- 	play_review AS (SELECT name, MAX(review_count) AS preview_count FROM play_store_apps
-- 				GROUP BY name
-- 				ORDER BY review_count)
-- SELECT * 
-- FROM play_review -- 
-- LEFT JOIN app_name, expected_revenue - marketing_costs - GREATEST(app_purchase_price, play_purchase_price) AS net_profit, apple_genre, play_genre, review_count, acontent_rating,  preview_count
-- -- FROM main_query
-- -- ORDER BY net_profit DESC, review_count::numeric DESC
